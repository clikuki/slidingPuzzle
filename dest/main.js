...
grid.style.setProperty('--size', pxIfy(getGridSize() * cellSize + (getGridSize() - 1) * cellGap));
function countInversions(cells) {
    let inversions = 0;
...
function isBeside(a, b) {
    if ([a - getGridSize(), a + getGridSize()].includes(b))
        return true;
    const aRow = getIndexRow(a);
    if (a - 1 === b && getIndexRow(a - 1) === aRow)
        return true;
    if (a + 1 === b && getIndexRow(a + 1) === aRow)
        return true;
}
function addCellListener(cell, i) {
    cell.setPosition(i);
    cell.elem.onclick = () => {
        if (isDoingWinAnimation)
            return;
            ...
            if (!hasWon &&
                ...
                promise.then(() => {
                    ...
                    cells.forEach((cell) => {
                        if (!cell)
                            return;
                        setTimeout(() => cell.doWinAnim().then(() => {
                            if (cell === lastCell)
                                isDoingWinAnimation = false;
                        }), Math.floor(cell.index / getGridSize() +
                            (cell.index % getGridSize())) * 150);
                    });
                });
            }
        }
...
let cells = new Array(getGridSize() ** 2 - 1)
    .fill(0)
...
listenToSizeChange((size) => {
    grid.style.setProperty('--size', pxIfy(size * cellSize + (size - 1) * cellGap));
